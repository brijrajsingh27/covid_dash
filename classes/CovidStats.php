...
//        print_r($countryCovidStats[0]['db_record']);print_r($countryCovidStats[0]['num_of_days']);
    ...
    public function getCurrentMonth() {
        ...
        $qry = "SELECT (SUM(new_infections) + SUM(new_infections)) as total FROM " . $this->table . " WHERE MONTH(last_updated) = MONTH(CURRENT_DATE()) AND YEAR(last_updated) = YEAR(CURRENT_DATE())
 ";
    ...
    public function getCurrentWeek() {
        ...
        $qry = "SELECT (SUM(new_infections) + SUM(new_infections)) as total FROM " . $this->table . " WHERE YEARWEEK(`last_updated`, 1) = YEARWEEK(CURDATE(), 1) ";
        $countryCovidStats = $this->db->query($qry);
        if ($countryCovidStats) {
            return $countryCovidStats[0]['total'];
        } else {
    ...
    public function getTodayStats() {
        ...
        $qry = "SELECT (SUM(new_infections) + SUM(new_infections)) as total FROM " . $this->table . " WHERE DATE(`last_updated`) = CURDATE() ";
